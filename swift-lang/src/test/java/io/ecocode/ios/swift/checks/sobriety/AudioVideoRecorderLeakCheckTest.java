...
public class AudioVideoRecorderLeakCheckTest {
    ...
    private static final String TEST_VIDEO_CASE_MISSING_RELEASE_CALL = "checks/sobriety/VideoRecorderLeak_no_trigger.swift";
    private static final String TEST_VIDEO_CASE_COMPLIANT = "checks/sobriety/VideoRecorderLeak_no_trigger.swift";
    ...
    @Test
    public void videoLeakCheck_missing_release_trigger() {
        SensorContextTester context = CheckTestHelper.analyzeTestFile(TEST_VIDEO_CASE_MISSING_RELEASE_CALL);
        assertThat(context.allIssues()).hasSize(1);
        Optional<Issue> issue = context.allIssues().stream().findFirst();
        issue.ifPresent(i -> {
            assertThat(i.ruleKey().rule()).isEqualTo(TESTED_RULE_ID);
            assertThat(i.ruleKey().repository()).isEqualTo(TEST_REPOSITORY);
            IssueLocation location = i.primaryLocation();
            assertThat(location.textRange().start().line()).isEqualTo(25);
        });
    }

    @Test
    public void videoLeakCheck_no_trigger() {
        SensorContextTester context = CheckTestHelper.analyzeTestFile(TEST_VIDEO_CASE_COMPLIANT);
        assertThat(context.allIssues()).isEmpty();
    }
